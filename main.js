...
conn.welcome = '*╭─ 〭〭〬〬⿻❥〬〬〬ꦿ[ ᴏʙɪᴛᴏ ] 〭〭〬〬⿻❥〬〬〬ꦿ─╮*\n*│ꪶ→ @subject*\n*│──────────────*\n*│ꪶ→ @user*\n*│ꪶ→ مرحبا انا بوت سفروت💦🌿✨* \n*│*\n*│ꪶ→ وصف💝 المجموعه:*\n\n@desc\n\n*│*\n*│ꪶ→ واتمنى ان تستمتع معنا بالمجموعه*\n*╰──────────────╯*'
...
setInterval(async () => {
  ...
  const bio = `🤖 مده النشاط: ${uptime} ┃ 👑 من صنع ♯ЅᗩFᏒOT꙯ ┃ 💬 حساب المطور: https://atom.bio/safrotbob-376`;
...
function clockString(ms) {
  ...
  return [d, ' سفروت(s) ️', h, ' البابا(s) ', m, ' بوت(s) ', s, ' الفقدان(s) '].map((v) => v.toString().padStart(2, 0)).join('');
}
